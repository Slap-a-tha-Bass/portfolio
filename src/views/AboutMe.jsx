import React from 'react';

const AboutMe = () => {
    return (
        <>
            <div className="col-md-12 mt-2 d-flex justify-content-center">
                <div>
                <h1 className="Projects text-center display-4 my-2">About me</h1>
                    <ul className="list-group">
                        <li className="list-group-item">Born and raised in Birmingham, AL</li>
                        <li className="list-group-item">Started learning TypeScript / React 1.5 years ago</li>
                        <li className="list-group-item">B.S. in Biology from Birmingham-Southern College</li>
                        <li className="list-group-item">Been in chemical sales for 7 years</li>
                        <li className="list-group-item">Daily chess player on chess.com</li>
                        <li className="list-group-item">Like to code at coffee shops but working on home office</li>
                        <li className="list-group-item">Wood working and DIY project friendly</li>
                        <li className="list-group-item">Unpaid part-time comedian</li>
                    </ul>
                </div>
            </div>
        </>
    )
}

export default AboutMe;
